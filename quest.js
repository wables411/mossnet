...
function buildTiles(th){const [g,gd,gl]=th.ground,[h,hd,hl]=th.ground2,[w,wl,wd]=th.water,[p,pd,pl]=th.path,[b,bd,bp]=th.wet,[ra,rb,rc]=th.rock,[ro,rl,rd]=th.road,[ma,mb,mc]=th.mtn;
  ...
  const bl=th.bldg;tl[BLDG]=[0,1,2,3].map(v=>tile(v===2?'#5a5048':v===3?'#f0ece0':bl.wall,q=>{q(0,0,v===2?'#3a3230':v===3?'#3f8f4a':bl.roof,16,4);q(0,4,'#1a1a2e',16,1);q(0,0,'#1a1a2e',1,16);q(15,0,'#1a1a2e',1,16);
...
let dealerAt=null,saveAt=null,saveMsg=null;
...
function placeDealer(ci){dealerAt=null;const r=rng(ci*977+3);
  ...
  if(cand.length){const i=(r()*cand.length)|0;dealerAt={x:cand[i][0],y:cand[i][1]};
    // the station goes in another building, as far from the Dealer as the town allows
    let best=null,bd=-1;for(const [x,y] of cand){const d=Math.abs(x-dealerAt.x)+Math.abs(y-dealerAt.y);if(d>bd){bd=d;best=[x,y];}}
    if(best&&bd>0)saveAt={x:best[0],y:best[1]};}}
...
function act(name,arg){
  ...
  else if(name==='shop:good'){const gd=JARGOODS.find(x=>x[0]===arg);if(!gd||jarHas(gd[0]))return;if(save.cheese<gd[2]){snd('miss');say(gd[2]+' CHEESE. NO CREDIT');return;}save.cheese-=gd[2];save.jar=save.jar||{};save.jar[gd[0]]=true;persist();snd('ok');say('IT IS IN THE JAR');dirty();}
  ...
  if(state==='title'){scene='lab';h=`<h1 class="q-title">moss quest</h1><p class="lcd-note q-center">${esc(SP.length)} species · seven continents · one MossDex</p>
    ...
    if(shop.mode==='menu'){const stock=dailyStock(c);const coat=roster[c].filter(x=>save.collected[x.key]).sort((A,B)=>(save.collected[B.key]||0)-(save.collected[A.key]||0)).slice(0,5);const rows=Math.max(stock.length,coat.length,1);
      ...
      let grid='';for(let i=0;i<rows;i++){const it=stock[i],sp=coat[i];
        ...
        grid+=sp?cell(`${sp.name} · ${SELLP[tierOf(sp,c)]}`,'shop:sell',sp.key,`A: sell for ${SELLP[tierOf(sp,c)]} cheese, press twice · ${TIERN[tierOf(sp,c)]}`):cell('—','shop:none',null,'nothing in the satchel','q-gone');}
      h=`<div class="q-dw"><div class="q-dw-top"><div class="q-dw-bank"><span class="q-dw-k">day</span><b>${day}</b><span class="q-dw-k">cheese</span><b class="q-dw-g">${save.cheese}</b><span class="q-dw-k">logged</span><b>${nCollected()}/${SP.length}</b><span class="q-dw-k">moss health</span><span class="q-dw-bar">${ph==null?'<i style="width:0"></i><em>no jar</em>':`<i style="width:${ph}%"></i><em>${ph}%</em>`}</span></div><div class="q-dw-city"><b>${esc(titleCase(CN[c]))}</b><span>the Dealer · abandoned house</span></div></div>
        ...
        <div class="grid q-dw-cols"><div class="q-dw-h">available moss</div><div class="q-dw-h">your satchel · ${roster[c].filter(x=>save.collected[x.key]).length}</div>${grid}</div>
    ...
    else if(shop.mode==='goods'){h=`<div class="lcd-header"><span class="lcd-header-title">JAR GOODS</span><span class="item-meta">${save.cheese} cheese</span></div><p class="lcd-note">for the terrarium. bought once, kept for every cutting after.</p><ul class="menu">${JARGOODS.map(gd=>jarHas(gd[0])?`<li><button type="button" class="menu-item q-gone" disabled>${esc(gd[1])} · yours</button></li>`:mi(gd[1]+' · '+gd[2]+' cheese','shop:good',gd[0],gd[3])).join('')}${mi('back','shop:back',null,'B does it too')}</ul>`;}
...
function drawTileAt(t,x,y,tx,ty){const base=OVER[t]?TL[G0]:null;if(base)ctx.drawImage(base,x,y);
  let img_=TL[t];if(t===WATER)img_=TL[WATER][(frame>>5)&1];else if(t===BLDG)img_=(dealerAt&&tx===dealerAt.x&&ty===dealerAt.y)?TL[BLDG][2]:(saveAt&&tx===saveAt.x&&ty===saveAt.y)?TL[BLDG][3]:TL[BLDG][(tx+ty)&1];ctx.drawImage(img_,x,y);}
